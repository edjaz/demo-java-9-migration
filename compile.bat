...
copy build\jars\monitor.utils.jar build\module-path
...
copy build\jars\monitor.observer.jar build\module-path
...
copy build\jars\monitor.observer.alpha.jar build\module-path
...
copy build\jars\monitor.observer.beta.jar build\module-path
...
copy build\jars\monitor.statistics.jar build\module-path
...
set JARS=
for %%f in (build\class-path\*.jar) do call set JARS=%%JARS%%;"%%f"
javac --class-path %JARS% --module-path build/module-path --add-modules monitor.statistics -d build/classes/monitor.persistence @sources.txt
del sources.txt
jar --create --file build/jars/monitor.persistence.jar -C build/classes/monitor.persistence .
copy build\jars\monitor.persistence.jar build\module-path

move build\class-path\spark-core* build\module-path
move build\class-path\jackson-core* build\module-path
move build\class-path\jackson-databind* build\module-path

echo " > creating monitor.rest"
dir /S /B monitor.rest\src\main\java\*.java > sources.txt
set JARS=
for %%f in (build\class-path\*.jar) do call set JARS=%%JARS%%;"%%f"
javac --class-path %JARS% --module-path build/module-path --patch-module java.xml.ws.annotation=build/class-path/jsr305-3.0.2.jar -d build/classes/monitor.rest @sources.txt
del sources.txt
jar --create --file build/jars/monitor.rest.jar -C build/classes/monitor.rest .
copy build\jars\monitor.rest.jar build\module-path
...
set JARS=
for %%f in (build\class-path\*.jar) do call set JARS=%%JARS%%;"%%f"
javac --class-path %JARS% --module-path build/module-path --add-modules monitor.observer.alpha,monitor.observer.beta --add-modules monitor.utils,monitor.statistics --add-modules monitor.rest -d build/classes/monitor @sources.txt
del sources.txt
jar --create --file build/jars/monitor.jar -C build/classes/monitor .
copy build\jars\monitor.jar build\module-path
